...
describe('투표 전략 서비스', () => {
  ...
  describe('투표 결정 계산', () => {
    it('마피아 플레이어가 시민을 타겟으로 선택해야 함', async () => {
      ...
      jest
        ...
        .mockReturnValue([mafiaPlayer, citizenPlayer, policePlayer]);
      ...
      const decision = await service.calculateVote(
        mafiaPlayer,
        persona,
        game,
        suspicionData,
      ...
      expect(mockLogger.log).toHaveBeenCalledWith(
        'Calculating vote for player Player1 (mafia)',
      );
    });
    ...
    it('시민 플레이어가 의심도가 높은 플레이어를 선택해야 함', async () => {
      ...
      jest
        .spyOn(game, 'getAlivePlayers')
        .mockReturnValue([citizenPlayer, suspectedPlayer, normalPlayer]);
    ...
    it('경찰 플레이어가 분석적 접근을 해야 함', async () => {
      ...
      jest
        .spyOn(game, 'getAlivePlayers')
        .mockReturnValue([policePlayer, suspectedMafia, citizenPlayer]);
    ...
    it('의사 플레이어가 신중한 투표를 해야 함', async () => {
      ...
      jest
        .spyOn(game, 'getAlivePlayers')
        .mockReturnValue([doctorPlayer, player2, player3]);
  ...
  describe('성격에 따른 투표 전략', () => {
    it('공격적인 성격이 더 높은 확신도를 가져야 함', async () => {
      ...
      const cautiousPersona = createMockPersona({
        aggression: 0.1,
        caution: 0.9,
      });
      ...
      const aggressiveDecision = await service.calculateVote(
        player,
        aggressivePersona,
        game,
        suspicionData,
      );
      const cautiousDecision = await service.calculateVote(
        player,
        cautiousPersona,
        game,
        suspicionData,
      ...
      expect(aggressiveDecision.confidence).toBeGreaterThan(
        cautiousDecision.confidence,
      );
    });
  ...
  describe('투표 설명 생성', () => {
    it('투표 설명이 성격에 맞게 생성되어야 함', async () => {
      ...
      const explanation = await service.generateVoteExplanation(
        decision,
        votingContext,
      );
      ...
      expect([
        ...
        'defensive',
        'aggressive',
      ]).toContain(explanation.emotionalTone);
    });
    ...
    it('높은 확신도에서는 자신감 있는 톤이어야 함', async () => {
      ...
      const confidentPersona = createMockPersona({
        aggression: 0.8,
        leadership: 0.9,
      });
  ...
  describe('투표 시뮬레이션', () => {
    it('투표 시뮬레이션이 여러 시나리오를 생성해야 함', async () => {
      ...
      jest
        .spyOn(game, 'getAlivePlayers')
        .mockReturnValue([player, target1, target2]);
      ...
      expect(mockLogger.log).toHaveBeenCalledWith(
        'Running voting simulation with 3 scenarios',
      );
    });
  });
  ...
  describe('역할별 전략 차이', () => {
    it('마피아와 시민의 투표 전략이 달라야 함', async () => {
      ...
      jest
        .spyOn(game, 'getAlivePlayers')
        .mockReturnValue([mafiaPlayer, citizenPlayer, targetPlayer]);

      const persona = createMockPersona();
      const suspicionData = new Map([[3, createMockSuspicionData(0.5)]]);

      const mafiaDecision = await service.calculateVote(
        mafiaPlayer,
        persona,
        game,
        suspicionData,
      );
      const citizenDecision = await service.calculateVote(
        citizenPlayer,
        persona,
        game,
        suspicionData,
      ...
      expect([citizenPlayer.id, targetPlayer.id]).toContain(
        mafiaDecision.target.id,
      );
      expect([citizenPlayer.id, targetPlayer.id]).toContain(
        citizenDecision.target.id,
      );
    });
    ...
    it('경찰이 조사 결과를 활용한 전략을 사용해야 함', async () => {
      ...
      jest
        .spyOn(game, 'getAlivePlayers')
        .mockReturnValue([policePlayer, suspectedPlayer]);
  ...
  describe('전략 업데이트 및 학습', () => {
    ...
    it('역할별 전략 업데이트가 정상 동작해야 함', () => {
      ...
      expect(mockLogger.log).toHaveBeenCalledWith(
        'Updating voting strategy for role: citizen',
      );
    });
  });
});
