...
export class SuspicionTrackerService {
  private readonly suspicionMaps = new Map<number, Map<number, Map<number, SuspicionData>>>();
  ...
  updateSuspicion(
    ...
    this.logger.log(`Updating suspicion: Player ${suspectingPlayerId} suspects Player ${suspectedPlayerId}`);

    const gameMap = this.getOrCreateGameMap(gameId);
    const playerSuspicions = this.getOrCreatePlayerSuspicions(gameMap, suspectingPlayerId);
    const currentSuspicion = this.getOrCreateSuspicionData(playerSuspicions, suspectedPlayerId);
    ...
    currentSuspicion.confidence = this.calculateConfidence(currentSuspicion.reasons);
  ...
  analyzeMessageForSuspicion(
    ...
    this.logger.log(`Analyzing message from player ${message.senderId} for suspicions`);
  ...
  analyzeNightSurvivalPattern(
    ...
    const survivalSuspicion = this.calculateSurvivalSuspicion(survivedPlayerId, game);
    
    if (survivalSuspicion > 0.3) { // 임계값 이상일 때만 의심 업데이트
      const reason: SuspicionReason = {
        type: 'night_survival',
  ...
  recordVote(
    ...
    this.logger.log(`Recording vote: Player ${voterId} votes for Player ${targetId}`);
  ...
  generateSuspicionInference(
    ...
    this.logger.log(`Generating suspicion inference for player ${targetPlayerId}`);
    ...
    const probability = this.calculateRoleProbability(suspicionData, inferredRole);
    const reasoning = this.generateReasoning(suspicionData);
    const confidence = this.calculateInferenceConfidence(suspicionData, reasoning);
   ...
   * 종합적인 의심 보고서를 생성합니다.
   */
  generateSuspicionReport(gameId: number, analyzingPlayerId: number, game: Game): SuspicionReport {
    this.logger.log(`Generating suspicion report for player ${analyzingPlayerId}`);
    ...
    for (const player of game.players) {
      if (player.id !== analyzingPlayerId && player.isAlive) {
        suspicionInferences.push(this.generateSuspicionInference(gameId, analyzingPlayerId, player.id));
      }
    }
   ...
   * 플레이어의 의심 데이터를 가져옵니다.
   */
  getSuspicionData(gameId: number, suspectingPlayerId: number, suspectedPlayerId: number): SuspicionData | null {
    const gameMap = this.suspicionMaps.get(gameId);
  ...
  cleanup(gameId: number): void {
    ...
    for (const [playerId, votes] of this.votingHistory) {
      const filteredVotes = votes.filter(vote => vote.gameContext.phase !== undefined);
  ...
  private getOrCreateGameMap(gameId: number): Map<number, Map<number, SuspicionData>> {
  ...
  private getOrCreatePlayerSuspicions(gameMap: Map<number, Map<number, SuspicionData>>, playerId: number): Map<number, SuspicionData> {
  ...
  private getOrCreateSuspicionData(playerSuspicions: Map<number, SuspicionData>, suspectedPlayerId: number): SuspicionData {
  ...
  private calculateLevelChange(intensity: number, currentSuspicion: SuspicionData): number {
  ...
  private calculateConfidence(reasons: SuspicionReason[]): number {
    ...
    const avgConfidence = reasons.reduce((sum, reason) => sum + reason.confidence, 0) / reasons.length;
  ...
  private detectSuspiciousElements(message: Message): Array<{description: string, intensity: number, confidence: number}> {
    const elements: Array<{description: string, intensity: number, confidence: number}> = [];
    const content = message.content.toLowerCase();

    // 방어적 언어 패턴 감지
    const defensivePatterns = ['아니야', '그런 게 아니야', '왜 나를', '증거가 있어?', '억울해'];
    ...
    // 주제 전환 시도 감지
    const redirectPatterns = ['그런데', '그보다는', '다른 얘기인데', '그것보다'];
  ...
  private calculateSurvivalSuspicion(survivedPlayerId: number, game: Game): number {
    // 간단한 생존 패턴 분석
    // 실제로는 더 복잡한 알고리즘이 필요
  ...
  private identifySuspiciousVotes(votingHistory: VoteRecord[], game: Game): SuspiciousVoteAnalysis[] {
  ...
  private calculateVotingPatternScore(votingHistory: VoteRecord[], game: Game): number {
    ...
    // 투표 확신도 평균
    const avgConfidence = votingHistory.reduce((sum, vote) => sum + vote.confidence, 0) / votingHistory.length;
    score += (avgConfidence - 0.5) * 0.3;
    
    // 근거 제시율
    const reasonProvisionRate = votingHistory.filter(vote => vote.reason).length / votingHistory.length;
    score += reasonProvisionRate * 0.2;
    
    return Math.max(0, Math.min(1, score));
  }

  private updateSuspicionBasedOnVote(gameId: number, voterId: number, targetId: number, vote: VoteRecord): void {
  ...
  private inferRole(suspicionData: SuspicionData): 'citizen' | 'mafia' | 'police' | 'doctor' | 'unknown' {
    if (suspicionData.level > 0.7) {
      return 'mafia';
    } else if (suspicionData.level < 0.3) {
      // 낮은 의심도는 시민이거나 특수 역할일 가능성
      const hasRoleClaimEvidence = suspicionData.reasons.some(r => r.type === 'role_claim');
  ...
  private calculateRoleProbability(suspicionData: SuspicionData, role: string): number {
    // 역할별 확률 계산
    switch (role) {
      case 'mafia':
  ...
  private calculateInferenceConfidence(suspicionData: SuspicionData, reasoning: any[]): number {
    const reasoningStrength = reasoning.reduce((sum, r) => sum + r.weight, 0) / reasoning.length;
  ...
  private generateOverallAssessment(
    ...
    const teamEstimate: TeamCompositionEstimate = {
      ...
      specialRoles: [
        {
          role: 'police',
          candidates: rankings.filter(r => r.suspicionScore < 0.3).map(r => ({
            playerId: r.playerId,
            probability: 0.3
          })),
          confidence: 0.4
    ...
    const gameAnalysis: GameStateAnalysis = {
      ...
      estimatedRemainingTurns: Math.ceil(game.players.filter(p => p.isAlive).length / 2),
