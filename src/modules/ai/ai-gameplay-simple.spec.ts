...
describe('AI 게임플레이 기본 테스트', () => {
  ...
  beforeEach(async () => {
    ...
    phaseBehaviorService = module.get<PhaseBehaviorService>(PhaseBehaviorService);
  ...
  describe('기본 게임 시나리오', () => {
    it('6인 게임에서 페이즈별 행동 패턴이 동작해야 함', async () => {
      ...
      const votingResults = await phaseBehaviorService.executePhaseStartBehaviors(
        game, 
        'voting', 
        aiPlayers, 
        personaMap
    ...
    it('페이즈 전환이 에러 없이 동작해야 함', async () => {
      ...
      // 밤 -> 낮 전환
      await expect(
        phaseBehaviorService.executePhaseTransition(game, 'night', 'day', aiPlayers, personaMap)
      ...
      // 낮 -> 투표 전환  
      await expect(
        phaseBehaviorService.executePhaseTransition(game, 'day', 'voting', aiPlayers, personaMap)
      ).resolves.not.toThrow();
    ...
    it('상황별 반응이 정상 동작해야 함', async () => {
      ...
      // 플레이어 제거 상황
      await expect(
        phaseBehaviorService.executeSituationResponse('player_eliminated', game, aiPlayers, personaMap)
      ).resolves.not.toThrow();
    ...
    it('역할별로 다른 행동 패턴을 보여야 함', async () => {
      ...
      const mafiaResults = await phaseBehaviorService.executePhaseStartBehaviors(
        game, 
        'day', 
        [mafiaPlayer], 
      ...
      const citizenResults = await phaseBehaviorService.executePhaseStartBehaviors(
        game, 
        'day', 
        [citizenPlayer], 
  ...
  describe('에러 처리', () => {
    ...
    it('페르소나가 없는 플레이어는 건너뛰어야 함', async () => {
      ...
      // 일부 플레이어만 페르소나 할당
      const partialPersonaMap = new Map([[1, createMockPersona('test-1', 'Player1')]]);
